...
type Goods struct {
	...
	DescImages      GormList `gorm:"type:varchar(1000);not null;comment:'商品展示图'"`
	GoodsFrontImage string   `gorm:"type:varchar(200);not null;comment:'封面图'"`
	Stocks          int32    `gorm:"type:int;default:0;not null;comment:'商品库存'"`
}
...
func (g *Goods) AfterCreate(tx *gorm.DB) (err error) {
	...
	_, err = global.EsConfig.Index().Index(EsGoods{}.GetIndexName()).BodyJson(goods).Id(strconv.Itoa(int(goods.ID))).Do(context.Background())
	if err != nil {
		return err
	}
	return nil
...
func (g *Goods) AfterUpdate(tx *gorm.DB) (err error) {
	...
	_, err = global.EsConfig.Update().Index(EsGoods{}.GetIndexName()).Doc(goods).Id(strconv.Itoa(int(goods.ID))).Do(context.Background())
	if err != nil {
		return err
	}
	return nil
...
func (g *Goods) AfterDelete(tx *gorm.DB) (err error) {
	_, err = global.EsConfig.Delete().Index(EsGoods{}.GetIndexName()).Id(strconv.Itoa(int(g.ID))).Do(context.Background())
	if err != nil {
		return err
	}
	return nil
